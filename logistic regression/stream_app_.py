...
# Define the features list that the model was trained on
# This list must match the order and names of features the model expects after preprocessing
# Based on the build_inputs function, these are the expected features.
features_list = [
...
def load_model(path='/content/logistic_model.pkl'):
    # Assuming the .pkl file directly contains the LogisticRegression pipeline object
    pipeline = joblib.load(path)
    return pipeline

pipeline = load_model('/content/logistic_model.pkl')
features = features_list # Assign the predefined list to 'features'
...
if submitted:
    ...
    st.write("Predicted class (0 = not survived, 1 = survived):")
